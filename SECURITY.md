...
2. Generate a fine-grained token scoped to the organization or repositories
   you want to audit
3. **Access needed** (read-only):
   - Metadata
   - Administration
   - Contents
   - Dependabot alerts
   - Secret scanning alerts
4. **Not needed**: write access to anything. Every API call this tool makes
   is a GET. On a classic token, do not grant `repo` - it is full read/write
...
See `requirements.txt` for pinned versions and `requirements-dev.txt` for
`pip-audit`, used to check pinned versions for known vulnerabilities.
