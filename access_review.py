...
2. **Findings**: the specific patterns an auditor challenges. Excessive
   ...
   push access. Findings flow through the same three-state model as every other
   check.
...
class AccessReview:
    ...
    @staticmethod
    def _elevated(principals, kinds=("user",)) -> List[Dict[str, Any]]:
        return [
            p for p in principals
            if p["kind"] in kinds and p["permission"] in ELEVATED
        ]
    ...
    def check_admin_concentration(self, inventory) -> Dict[str, Any]:
        ...
        if not admins:
            return ok("No principal holds repository admin beyond the owner")

        if len(users) > self.ADMIN_COUNT_THRESHOLD:
            names = ", ".join(p["name"] for p in users[:5])
            return fail(
    ...
    def check_owner_count(self) -> Dict[str, Any]:
        ...
        if count < self.OWNER_COUNT_MIN:
            return fail(
                ...
                "and security settings. Auditors raise this as a availability finding"
            )
