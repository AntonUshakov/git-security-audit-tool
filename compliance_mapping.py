...
def scope_results_to_standard(audit_results, standard):
    ...
    def prune(bucket):
        ...
        for check_name, result in kept.items():
            status = result.get(
    ...
    weighted_score = (
        ...
        if weighted_evaluated else score
    )
    coverage = round(evaluated / total * 100, 2) if total else 0.0
    pass_rate_of_total = round(passed / total * 100, 2) if total else 0.0
    ...
    scoped["summary"] = {
        ...
        "risk_level": _risk_level(weighted_score) if evaluated else "UNSCORED",
    }
...
# ---------------------------------------------------------------------------
#
# A flat pass/fail percentage treats "no build provenance" the same as "2FA
# is off". Every check contributes an equal 1/N of the score regardless of what
# its failure actually enables. Severity weighting fixes that: critical findings
# count more than low ones in the weighted score and in risk-level classification.
#
# Assigned once, by what a failure of that specific control enables - not by
# framework, not by how many frameworks map to it. Revisit if real-world usage
# shows a different control matters more or less than assigned here.

SEVERITY = {
    '2FA Enforcement': 'critical',
    'Branch Protection Rules': 'critical',
    'Ruleset Enforcement Status': 'critical',
    'Secrets Scanning': 'critical',
    'Push Protection': 'critical',
    'Dependency Scanning': 'critical',
    'Actions Allowed Actions Policy': 'critical',
    'Repository Actions Policy': 'critical',
    'Action Version Pinning': 'critical',
    'Self-Hosted Runner Exposure': 'critical',
    'Pull Request Reviews': 'high',
    'Admin Bypass Prevention': 'high',
    'Actions Default Token Permissions': 'high',
    'Actions Pull Request Approval': 'high',
    'Workflow Token Permissions': 'high',
    'Action SHA Pinning Policy': 'high',
    'Untrusted Workflow Triggers': 'high',
    'Workflow Permissions Declared': 'high',
    'Fork Pull Request Workflows': 'high',
    'Direct Collaborator Grants': 'high',
    'Outside Collaborator Access': 'high',
    'Repository Admin Concentration': 'high',
    'Organization Owner Count': 'high',
    'Default Repository Permission': 'high',
    'Member Repository Creation': 'medium',
    'Status Checks Before Merge': 'medium',
    'Dismiss Stale PR Reviews': 'medium',
    'Code Owner Reviews': 'medium',
    'Linear History Required': 'medium',
    'Force Push Protection': 'medium',
    'Branch Deletion Protection': 'medium',
    'Commit Signing': 'medium',
    'Repository Visibility': 'medium',
    'SSO Configuration': 'medium',
    'Audit Logging': 'medium',
    'Build Provenance Attestation': 'medium',
    '.gitignore Configuration': 'low',
    'SECURITY.md File': 'low',
    'CODEOWNERS File': 'low',
    'Repository Activity': 'low',
}

#: Numeric weight per severity, used to compute the weighted score.
#: Critical counts four times as much as low - not so much that a single
#: critical failure can never be offset by fixing everything else, but enough
#: that it can't be diluted into invisibility by counting equally with 39 others.
SEVERITY_WEIGHT = {"critical": 4, "high": 3, "medium": 2, "low": 1}


def severity_for(check_name: str) -> str:
    """Severity of a check, defaulting to 'medium' for an unmapped name
    rather than raising - an unrecognised name should degrade gracefully,
    not crash the summary calculation."""
    return SEVERITY.get(check_name, "medium")
...
# ---------------------------------------------------------------------------
#
# Who can act on a failing control - not a named person (this tool has no way
# to know who that is at a given organization), but the class of access
# required to fix it. A gap table sorted only by severity still leaves the
# reader to work out whether a finding needs a security admin, an org owner,
# or is simply a pull request anyone on the team can open.
#
# Plan-restricted findings (see reason_category) are not in this table at
# all - they have their own dedicated report section, since their actual
# owner is a billing/plan decision, not any of the three categories below.

OWNER_CATEGORY = {
    '2FA Enforcement': 'Organization owner',
    'SSO Configuration': 'Organization owner',
    'Default Repository Permission': 'Organization owner',
    'Member Repository Creation': 'Organization owner',
    'Audit Logging': 'Organization owner',
    'Actions Allowed Actions Policy': 'Organization owner',
    'Actions Default Token Permissions': 'Organization owner',
    'Actions Pull Request Approval': 'Organization owner',
    'Organization Owner Count': 'Organization owner',
    'Repository Visibility': 'Repository admin',
    'Branch Protection Rules': 'Repository admin',
    'Pull Request Reviews': 'Repository admin',
    'Status Checks Before Merge': 'Repository admin',
    'Commit Signing': 'Repository admin',
    'Dismiss Stale PR Reviews': 'Repository admin',
    'Code Owner Reviews': 'Repository admin',
    'Admin Bypass Prevention': 'Repository admin',
    'Linear History Required': 'Repository admin',
    'Force Push Protection': 'Repository admin',
    'Branch Deletion Protection': 'Repository admin',
    'Secrets Scanning': 'Repository admin',
    'Push Protection': 'Repository admin',
    'Dependency Scanning': 'Repository admin',
    'Repository Activity': 'Repository admin',
    ...
    'Ruleset Enforcement Status': 'Repository admin',
    'Direct Collaborator Grants': 'Repository admin',
    'Outside Collaborator Access': 'Repository admin',
    'Repository Admin Concentration': 'Repository admin',
    'SECURITY.md File': 'Engineering team',
    'CODEOWNERS File': 'Engineering team',
    '.gitignore Configuration': 'Engineering team',
    'Action Version Pinning': 'Engineering team',
    'Workflow Permissions Declared': 'Engineering team',
    'Untrusted Workflow Triggers': 'Engineering team',
    'Self-Hosted Runner Exposure': 'Engineering team',
    'Build Provenance Attestation': 'Engineering team',
}


def owner_category_for(check_name: str) -> str:
    """Defaults to 'Repository admin' for an unmapped name - the most common
    category and the safer assumption (asking a lower-privilege person to try
    a fix that actually needs admin wastes less time than the reverse)."""
    return OWNER_CATEGORY.get(check_name, "Repository admin")
