...
import secrets
from github_auditor import (
    GitHubAuditor, resolve_github_target,
    GITHUB_RETRY_POLICY, configure_rate_limit_logging,
)
from history import build_history_entry, find_previous_run, diff_runs
...
def _safe_error(exc: Exception) -> str:
    ...
    return f"GitHub request failed: {type(exc).__name__}. See server logs for details."
...
def create_app(org=None):
    ...
    if history_file.exists():
        try:
            ...
            # A corrupted history file (interrupted write, disk full, manual
            # edit gone wrong) must not prevent the app from starting - that
            # turns a recoverable data problem into total unavailability.
            # Preserve the bad file for inspection rather than overwriting it
            # silently on the next successful write.
            backup = history_file.with_suffix(".json.corrupted")
            try:
                history_file.rename(backup)
                print(f"[WARN] audit_results/history.json was corrupted ({exc}); "
                      f"moved to {backup.name} and starting with empty history.")
            except OSError:
                print(f"[WARN] audit_results/history.json was corrupted ({exc}); "
    ...
    def index():
        ...
        return render_template_string(LANDING_PAGE_TEMPLATE, version=version_string())
    
    @app.route('/start')
    def start_audit_page():
        """Page to start new audit"""
        return render_template_string(START_AUDIT_TEMPLATE, version=version_string())
    
    @app.route('/api/list-repositories', methods=['POST'])
    def list_repositories():
        """
        List repositories for the account/organization, for the "find
        repositories, then pick which to audit" flow in the start form.

        Read-only and synchronous - a single paginated GitHub API call, not a
        background thread like the audit itself, since there is no per-repo
        work to do here.
        """
        ...
        # Cleared as soon as it is no longer needed in this scope, same
        # discipline as /api/start-audit.
        data['token'] = None

        if not github_token or not org_name:
            return jsonify({"error": "Token and organization name required"}), 400

        #: Hard cap so a very large organization cannot turn a "find
        #: repositories" click into an unbounded, slow API crawl. High enough
        #: that no realistic organization audited by hand hits it.
        MAX_REPOS = 1000

        try:
            gh = Github(github_token, retry=GITHUB_RETRY_POLICY)
            target, warning = resolve_github_target(gh, org_name, account_type)
            ...
            github_token = None
            return jsonify({
                "error": f"Failed to list repositories: {type(exc).__name__}"
            }), 500
    ...
    def start_audit():
        ...
        repo_names = data.get('repos') or None
        if repo_names is not None and not isinstance(repo_names, list):
            # Accept the same comma-separated format the CLI's --repos flag
            # takes. The web form's JS already splits before sending, but the
            # API endpoint itself must not depend on that - any other caller
            # sending a raw string here (curl, a script, a future
            # integration) would otherwise have the whole string wrapped into
            # one bogus repository name that matches nothing, silently
            # scoping the audit to zero repositories.
            repo_names = [r.strip() for r in str(repo_names).split(",") if r.strip()]
        elif repo_names is not None:
            repo_names = [str(r).strip() for r in repo_names if str(r).strip()]
        ...
        # `data` is the raw parsed request body and still holds the token in
        # its own dict entry. Clearing the key here, rather than only the
        # local `github_token` name, ensures nothing reachable from this
        # request object still carries the credential once we hand off to the
        # background thread.
        data['token'] = None
        ...
        def run_audit(token):
            ...
            try:
                ...
                results = auditor.audit_organization(verbose=False, repo_names=repo_names)
                
                results["standard_requested"] = standard

                # Compare against the most recent prior run for this org
                # BEFORE writing the new entry to history - otherwise the
                # "previous" run would be the one just completed.
                with _history_lock:
                    previous_run = find_previous_run(
                        audit_history, org_name, results.get("account_type"),
                        exclude_session_id=session_id,
                    )
                if previous_run:
                    new_entry = build_history_entry(results)
                    results["comparison"] = diff_runs(previous_run, new_entry)
                ...
                # Save to history
                with _history_lock:
                    audit_history[session_id] = build_history_entry(results)
                ...
                # Drop the parameter reference before the thread exits. The
                # outer `github_token` name and the request body's own copy
                # were already cleared above, before the thread was started.
                token = None
                del auditor
        ...
        thread = threading.Thread(target=run_audit, args=(github_token,))
        thread.start()
        # Drop the local name in this request-handling scope too. The thread
        # already has its own copy via `args`.
        github_token = None
    ...
    def wiki():
        """Documentation wiki"""
        return render_template_string(WIKI_TEMPLATE, version=version_string())
    ...
    def wiki_page(page):
        ...
        return render_template_string(WIKI_PAGE_TEMPLATE, page=page, content=content, current_page=page, version=version_string())
    
    @app.route('/history')
    def history():
        """View audit history, grouped by organization with a score trend."""

        def risk_css(audit):
            risk_level = (audit.get("summary") or {}).get("risk_level")
            if risk_level:
                return {
                    "LOW RISK": "score-high", "MEDIUM RISK": "score-medium",
                    "HIGH RISK": "score-low", "CRITICAL RISK": "score-low",
                }.get(risk_level, "score-low")
            # Entries written before this field existed have no risk_level at
            # all - the raw score is the only signal available for them, and
            # this fallback applies to that legacy data only. Every entry
            # written by build_history_entry() always includes risk_level.
            score = audit.get("score")
            if score is None:
                return ""
            return "score-high" if score >= 90 else "score-medium" if score >= 70 else "score-low"

        def trend_arrow(runs, index):
            if index + 1 >= len(runs):
                return ""
            current = (runs[index][1].get("summary") or {}).get("weighted_score")
            previous = (runs[index + 1][1].get("summary") or {}).get("weighted_score")
            if current is None or previous is None:
                return ""
            delta = round(current - previous, 1)
            if delta > 0:
                return f'<span style="color: #3fb950;">&#9650; {delta:+.1f}</span>'
            if delta < 0:
                return f'<span style="color: #f85149;">&#9660; {delta:+.1f}</span>'
            return '<span style="color: #8b949e;">&#8212;</span>'

        grouped = {}
        for session_id, entry in audit_history.items():
            org = entry.get("org") or "(unknown)"
            grouped.setdefault(org, []).append((session_id, entry))
        for org in grouped:
            grouped[org].sort(key=lambda pair: pair[1].get("timestamp") or "", reverse=True)
        grouped_history = dict(sorted(
            grouped.items(),
            key=lambda pair: pair[1][0][1].get("timestamp") or "", reverse=True
        ))

        return render_template_string(
            HISTORY_TEMPLATE, history=audit_history, grouped_history=grouped_history,
            risk_css=risk_css, trend_arrow=trend_arrow, version=version_string(),
        )
...
<body>
    ...
    <main class="container">
        ...
        <form id="auditForm">
            ...
            <div class="form-group">
                ...
                <p style="font-size: 0.9em; color: #8b949e; margin-bottom: 10px;">
                    Find your repositories and pick which ones to audit, or leave this
                    step out entirely to audit every repository. Organization-level
                    checks always run regardless of this scope.
                </p>
                <button type="button" id="findReposBtn"
                        style="width: 100%; background: #21262d; color: #e6edf3;
                               border: 1px solid #30363d; padding: 10px 20px;
                               border-radius: 4px; cursor: pointer; font-size: 1em;
                               margin-top: 0;">
                    Find Repositories
                </button>
                <p id="findReposError" style="color: #f85149; font-size: 0.9em; margin-top: 8px; display: none;"></p>
                <p id="findReposWarning" style="color: #d29922; font-size: 0.9em; margin-top: 8px; display: none;"></p>

                <div id="repoPicker" style="display: none; margin-top: 14px;">
                    <input type="text" id="repoFilter" placeholder="Filter repositories..."
                           style="margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <div style="font-size: 0.9em; color: #8b949e;">
                            <span id="repoSelectedCount">0</span> of <span id="repoTotalCount">0</span> selected
                        </div>
                        <div>
                            <button type="button" id="repoSelectAll"
                                    style="width: auto; margin: 0 0 0 8px; padding: 4px 10px; font-size: 0.85em;
                                           background: #21262d; border: 1px solid #30363d; color: #79c0ff;
                                           border-radius: 4px; cursor: pointer;">Select all</button>
                            <button type="button" id="repoSelectNone"
                                    style="width: auto; margin: 0 0 0 8px; padding: 4px 10px; font-size: 0.85em;
                                           background: #21262d; border: 1px solid #30363d; color: #79c0ff;
                                           border-radius: 4px; cursor: pointer;">Select none</button>
                        </div>
                    </div>
                    <div id="repoList"
                         style="max-height: 320px; overflow-y: auto; border: 1px solid #30363d;
                                border-radius: 4px; background: #0d1117;">
                    </div>
                    <p id="repoTruncatedNote" style="font-size: 0.85em; color: #8b949e; margin-top: 8px; display: none;">
                        Showing the first 1000 repositories.
                    </p>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <p style="font-size: 0.9em; color: #8b949e; margin-top: 8px;">
                    The name exactly as it appears in your GitHub URL. Enter this and
                    your token above before clicking "Find Repositories".
                </p>
            </div>
    ...
    <script>
        let allRepos = [];
        let selectedRepos = null;  // null = "everything" (picker never used)

        function renderRepoList(filterText) {
            const container = document.getElementById('repoList');
            const filter = (filterText || '').toLowerCase();
            const visible = allRepos.filter(r => r.name.toLowerCase().includes(filter));

            container.innerHTML = visible.map(r => {
                const checked = selectedRepos === null || selectedRepos.has(r.name);
                const badges = [
                    r.private ? '<span style="color:#d29922;">private</span>' : '<span style="color:#3fb950;">public</span>',
                    r.archived ? '<span style="color:#8b949e;">archived</span>' : ''
                ].filter(Boolean).join(' &middot; ');
                return `
                    <label style="display: flex; align-items: flex-start; gap: 10px; padding: 8px 12px;
                                  border-bottom: 1px solid #21262d; cursor: pointer; font-weight: normal;">
                        <input type="checkbox" class="repo-checkbox" value="${r.name}" ${checked ? 'checked' : ''}
                               style="width: auto; margin-top: 3px;">
                        <span>
                            <div>${r.name}</div>
                            <div style="font-size: 0.8em; color: #8b949e;">${badges}${r.description ? ' &middot; ' + r.description : ''}</div>
                        </span>
                    </label>`;
            }).join('') || '<div style="padding: 12px; color: #8b949e;">No repositories match.</div>';

            container.querySelectorAll('.repo-checkbox').forEach(cb => {
                cb.addEventListener('change', updateSelectedFromCheckboxes);
            });
            updateSelectedCount();
        }
        ...
        function updateSelectedFromCheckboxes() {
            if (selectedRepos === null) {
                selectedRepos = new Set(allRepos.map(r => r.name));
            }
        ...
        function updateSelectedCount() {
            const count = selectedRepos === null ? allRepos.length : selectedRepos.size;
            document.getElementById('repoSelectedCount').textContent = count;
            document.getElementById('repoTotalCount').textContent = allRepos.length;
        }
        ...
        document.getElementById('repoSelectAll').addEventListener('click', () => {
            selectedRepos = new Set(allRepos.map(r => r.name));
            renderRepoList(document.getElementById('repoFilter').value);
        });

        document.getElementById('repoSelectNone').addEventListener('click', () => {
            selectedRepos = new Set();
            renderRepoList(document.getElementById('repoFilter').value);
        });

        document.getElementById('findReposBtn').addEventListener('click', async () => {
            const token = document.getElementById('token').value;
            const org = document.getElementById('org').value;
            const accountType = document.getElementById('account_type').value;
            const errorEl = document.getElementById('findReposError');
            const warningEl = document.getElementById('findReposWarning');
            errorEl.style.display = 'none';
            warningEl.style.display = 'none';

            if (!token || !org) {
                errorEl.textContent = 'Enter your token and organization/username first.';
                errorEl.style.display = 'block';
                return;
            }

            const btn = document.getElementById('findReposBtn');
            btn.disabled = true;
            btn.textContent = 'Finding repositories...';

            try {
                const response = await fetch('/api/list-repositories', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, org, account_type: accountType })
                });
                const data = await response.json();
                if (!response.ok) throw new Error(data.error || 'Failed to list repositories');

                allRepos = data.repositories;
                selectedRepos = null;  // default: everything selected
                document.getElementById('repoPicker').style.display = allRepos.length ? 'block' : 'none';
                document.getElementById('repoTruncatedNote').style.display = data.truncated ? 'block' : 'none';
                if (data.warning) {
                    warningEl.textContent = data.warning;
                    warningEl.style.display = 'block';
                }
                if (!allRepos.length) {
                    errorEl.textContent = 'No repositories found for this account.';
                    errorEl.style.display = 'block';
                }
                renderRepoList('');
            } catch (error) {
                ...
                btn.disabled = false;
                btn.textContent = 'Find Repositories';
            }
        });
        ...
        document.getElementById('auditForm').addEventListener('submit', async (e) => {
            ...
            // null (picker never used, or every repo left checked) audits
            // everything; an explicit selection - including an empty one -
            // is sent as a list.
            const repos = selectedRepos === null ? null : Array.from(selectedRepos);
            try {
                const response = await fetch('/api/start-audit', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, org, account_type: accountType, standard, repos })
                });
    ...
    <footer style="border-top: 1px solid #30363d; padding: 24px 0; margin-top: 40px; color: #8b949e; text-align: center; font-size: 0.85em;">
        {{ version }}
    </footer>
</body>
</html>
...
<body>
    ...
    <main class="container">
        <div class="score-card">
            <div class="score-number">{{ results['summary'].get('weighted_score', results['summary']['compliance_score']) }}%</div>
            <div class="score-label">Overall Weighted Compliance Score</div>
            <div class="score-label" style="margin-top: 10px; font-size: 1.1em;">
                {% set rl = results['summary']['risk_level'] %}
                <span class="risk-{{ 'high' if rl == 'LOW RISK' else 'medium' if rl == 'MEDIUM RISK' else 'low' }}">
                    {{ rl }}
                </span>
            </div>
        </div>
        ...
        {% set comp = results['comparison'] %}
        <div style="background: #161b22; border: 1px solid #30363d; border-left: 4px solid #1f6feb; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
            <div style="font-weight: 600; margin-bottom: 10px;">
                Changes since the previous audit ({{ comp['previous_timestamp'][:10] if comp.get('previous_timestamp') else '' }}):
                {% set delta = comp.get('weighted_score_delta') %}
                {% if delta and delta > 0 %}<span style="color: #3fb950;">+{{ delta }} pts</span>
                {% elif delta and delta < 0 %}<span style="color: #f85149;">{{ delta }} pts</span>
                {% else %}<span style="color: #8b949e;">no change</span>{% endif %}
            </div>
            {% for repo_name, diff in comp.get('repository_diffs', {}).items() %}
            <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #21262d; font-size: 0.9em;">
                <strong>{{ repo_name }}</strong>
                {% if diff['newly_failing'] %}<div style="color: #f85149;">&#9660; Newly failing: {{ diff['newly_failing']|join(', ') }}</div>{% endif %}
                {% if diff['newly_passing'] %}<div style="color: #3fb950;">&#9650; Newly passing: {{ diff['newly_passing']|join(', ') }}</div>{% endif %}
                {% if diff['coverage_changed'] %}<div style="color: #8b949e;">Coverage changed: {{ diff['coverage_changed']|join('; ') }}</div>{% endif %}
            </div>
            {% endfor %}
            {% if comp.get('new_repositories') %}<div style="margin-top: 8px; font-size: 0.85em; color: #8b949e;">New since last audit: {{ comp['new_repositories']|join(', ') }}</div>{% endif %}
            {% if comp.get('removed_repositories') %}<div style="margin-top: 4px; font-size: 0.85em; color: #8b949e;">No longer in scope: {{ comp['removed_repositories']|join(', ') }}</div>{% endif %}
            {% if not comp.get('repository_diffs') and comp.get('unchanged_repository_count') %}
            <div style="margin-top: 8px; font-size: 0.9em; color: #8b949e;">No check changed status in any repository audited both times.</div>
            {% endif %}
        </div>
        {% endif %}
...
<body>
    ...
    <main class="container">
        {% if history %}
        {% for org, runs in grouped_history.items() %}
        <div style="margin-bottom: 32px;">
            <h2 style="color: #e6edf3; font-size: 1.3em; margin-bottom: 12px;">{{ org }}</h2>
            <table>
                <thead>
                    <tr>
                        <th>Timestamp (UTC)</th>
                ...
                <tbody>
                    {% for id, audit in runs %}
                    <tr>
                        <td>{{ audit['timestamp'] }}</td>
                        <td class="{{ risk_css(audit) }}">
                            {{ audit['summary']['weighted_score'] if audit.get('summary') else audit.get('score') }}%
                            {{ trend_arrow(runs, loop.index0) | safe }}
                        </td>
                        <td>{{ audit['summary']['risk_level'] if audit.get('summary') else '' }}</td>
                        <td>{{ audit['repositories']|length if audit.get('repositories') is not none else '\u2014' }}</td>
