...
**Usage:**
```bash
python3 github_auditor.py --org ORG               # CLI mode (GITHUB_TOKEN env var)
python3 app.py                                     # Web mode
```
...
See `requirements.txt` for the current pinned versions - this file is
rewritten by hand and can drift from it, so treat `requirements.txt` as
authoritative.
...
```bash
export GITHUB_TOKEN=ghp_your_token_here
python3 github_auditor.py --org YOUR_ORG --web
...
### "Permission denied"
- Token needs organization owner access for organization-level checks
...
1. **Get GitHub Token**
   ...
   - Use a fine-grained token, read-only: Metadata, Administration, Contents,
     Dependabot alerts, Secret scanning alerts. Do not grant `repo` - it is
     full write access and this tool never writes anything.
