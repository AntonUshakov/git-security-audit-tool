...
23 checks (5 organization-level, 18 repository-level) executed against the
real GitHub REST API via PyGithub.

Every check returns a three-state result:

    {"status": "pass" | "fail" | "unknown", "passed": bool, "message": str}

"unknown" means the check could not be evaluated (missing permission, plan
restriction, API error). Unknown results are EXCLUDED from the compliance
score rather than counted as failures — a missing permission is not a
security finding.
"""
...
def not_applicable(message: str) -> Dict[str, Any]:
    """
    The control cannot apply here (plan restriction, or a prerequisite control
    is already reported as failing). Excluded from the score so that one root
    cause produces one finding instead of ten.
    """
    return _result(NOT_APPLICABLE, message)
...
class ProtectionFacts:
    ...
    #: Rules that govern the repository or organization, not a branch. Observed
    #: in a /rules/branches/{b} response on hashicorp/terraform.
...
class SecurityChecker:
    ...
    def _enforcement(self, repo) -> str:
        ...
        The same banner appears on Settings > Actions > Policies, because
        Actions policies are implemented as rulesets. The restriction is
        systemic across ruleset-backed features, not specific to branches.
    ...
    def _check_2fa_enforcement(self) -> Dict[str, Any]:
        ...
        return fail(
                "Organization does not require 2FA for membership. Platform-wide 2FA "
                "covers github.com contributors, but this policy is what blocks a "
                "non-compliant account from being added — enable at org/settings/security"
            )
    ...
    def _check_audit_logs(self) -> Dict[str, Any]:
        """Audit log API is Enterprise Cloud only and absent from PyGithub 2.1.1."""
    ...
    def _get_protection(self, repo) -> "ProtectionFacts":
        ...
        if enforcement == "not_enforced":
            # Configuration may exist, but the plan does not honour it.
            facts.state = (
                "configured_not_enforced" if (classic_ok or ruleset_ok)
                else "not_enforceable"
            )
            return facts
    ...
    def _api_get(self, obj, path: str, label: str = "Setting"):
        """GET an arbitrary API path using an object's requester."""
        try:
            requester = obj._requester
        except AttributeError:
            return None, "no requester available"
        try:
            _, data = requester.requestJsonAndCheck("GET", path)
            return data, None
        except Exception as exc:
            return None, _from_exception(exc, label)["message"]
    ...
    def _check_ruleset_enforcement(self, repo) -> Dict[str, Any]:
        ...
        if not data:
            return not_applicable("No rulesets are defined for this repository")
        ...
        return ok(f"All {len(data)} ruleset(s) are actively enforced")
